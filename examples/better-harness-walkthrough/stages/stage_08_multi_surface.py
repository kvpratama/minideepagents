...
SURFACES = [
    Surface(
        name="prompt",
        target="stages.stage_08_multi_surface:BASE_PROMPT",
        base_value=BASE_PROMPT,
        filename="prompt.txt",
    ),
    Surface(
        name="calculator_guidance",
        target="stages.stage_08_multi_surface:CALCULATOR_GUIDANCE",
...
def make_calculator_with_guidance() -> object:
...
def build_inner_agent():
    ...
    return create_agent(model, tools=[calc_tool], prompt=BASE_PROMPT)
