...
└── stages/
    ├── stage1_basic_agent.py        + .md   # bare LangChain create_agent
    ├── stage2_thinking_tool.py      + .md   # + think_tool, system prompt
    ├── stage3_deep_agent.py         + .md   # → create_deep_agent
    ├── stage4_subagent.py           + .md   # + research-agent sub-agent
    └── stage5_full_orchestrator.py  + .md   # full real-example prompts
```

Each `stageN_*.py` exports a module-level `agent` graph **and** has a CLI
...
uv run python stages/stage5_full_orchestrator.py "Compare DuckDB vs SQLite"
```
...
Studio's graph dropdown lists `stage1_basic_agent` through
`stage5_full_orchestrator`. Pick any one to run and inspect.

## Reading order

1. Run stage 1 once. Read [`stage1_basic_agent.md`](stages/stage1_basic_agent.md).
2. Run stage 2. Read [`stage2_thinking_tool.md`](stages/stage2_thinking_tool.md).
...
After stage 5, diff `stages/stage5_full_orchestrator.py` against
